    ...
    public class SeasonSummaryReport : AbstractReport {
        ...
        private string MakeTimeRangeString(string currentClass, List<Contest> contests) {
            ...
            foreach (var contest in contests) {
                if (contest.RaceClass.Equals(currentClass)) {
                    if (contest.ContestDate < lowDate) {
                        lowDate = contest.ContestDate;
                    }

                    if (contest.ContestDate > highDate) {
                        highDate = contest.ContestDate;
                    }
                }
            }

            return lowDate.ToString("MMMM d, yyyy") + " to " + highDate.ToString("MMMM d, yyyy");
        }
        ...
        private void WriteDetails(string currentClass, string timeRange, Dictionary<string, LineItem> dict) {
            AddLine(currentClass);
            AddLine(timeRange);
            AddLine("");
